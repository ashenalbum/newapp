...
(function(w){
	...
	w[_namespace] = function(){
		...
		var _args = arguments[0];
			...
			_historys = [],
			_placeHolder = _args.placeHolder || '<p class="Eleditor-placeholder">点击此处添加内容</p>',
			_uploadRole = null;
			...
			var _uploaderHanlder = function(){
				...
				try{
					_args.uploader.call(this).then(function(_url){
                        ...
                        // 自定义
                        if(_$wrap.find(".Eleditor-placeholder").length==0){
                            _$wrap.append('<p class="Eleditor-placeholder">+ 继续添加内容</p>');
                        }
						_args.changer();
					}, function(_err){
						w.alert('上传失败：['+_err+']');
